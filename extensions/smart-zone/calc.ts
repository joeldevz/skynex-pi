 ...
 * @param warning_threshold effective warning threshold
 * @param hard_cap          effective hard cap threshold
 * @param lastWarnedAt      highest token count at which we already warned this session
 *                          (pass 0 if never warned)
 * @param warning_step      increment in tokens between repeated warnings
 */
export function decideAction(
  ...
  const percent_of_cap = Math.min(100, Math.round((tokens / hard_cap) * 100));
  ...
  if (tokens >= warning_threshold) {
    const nextWarn = Math.max(warning_threshold, lastWarnedAt + warning_step);
    if (tokens >= nextWarn) {
      return {
        action: "warn",
        tokens,
 ...
 */
export function formatStatusLine(tokens: number, hard_cap: number): string {
  const pct = Math.min(100, Math.round((tokens / hard_cap) * 100));
  const bar = formatBar(pct, 10);
  return `tokens ${formatTokens(tokens)}/${formatTokens(hard_cap)} ${bar}`;
}
...
export function buildCheckpointContent(
  ...
  return [
    ...
    ``,
    `## Context`,
    `- Token usage: ${formatTokens(tokenUsage)} / ${formatTokens(hard_cap)}`,
