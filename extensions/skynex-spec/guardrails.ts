...
export function detectActiveDomainCollisions(
	...
	const changesDir = join(cwd, ".skynex", "changes");
...
export function detectLegacyFlatSpec(
	...
	const changeDir = join(cwd, ".skynex", "changes", changeName);
	const path = join(changeDir, "spec.md");
	if (!existsSync(path)) return undefined;
	return { change: changeName, path, hasDomainSpecs: hasAnyDomainSpec(join(changeDir, "specs")) };
}
...
export function analyzeDeltaDestructiveness(
	...
	const largeModifiedRequirements = delta.modified
		.map((block) => ({ name: block.name, lineCount: block.content.split("\n").length }))
