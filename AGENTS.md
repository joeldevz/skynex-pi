...
This repo is in **design + infrastructure phase**. All current TypeScript code lives in `extensions/` (Pi runtime extensions) and `assets/agents/` (sub-agent definitions). There is intentionally **no `src/` directory** — it is gitignored to prevent E2E test residues from being tracked.
...
Sub-agents are **trusted, version-controlled** files under `assets/agents/*.md`. When invoking the `subagent` tool, always pass `confirmProjectAgents: false` to skip the per-call confirmation dialog (only needed for unfamiliar repos).
...
- `pi-web-access` (`npm:pi-web-access`) — Web search + URL fetch tool. Registers `web_search`, `fetch_content`, `code_search`. Zero-config via Exa MCP. Used by scout during discovery and via `/skill:research` for deep research.
